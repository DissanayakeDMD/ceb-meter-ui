...
const BULK_API =
  "https://localhost:7221/api/process/GetLastBulkFive";
...
/** Same layout as ProcessTable; loads bulk process rows from GetLastBulkFive. */
function ProcessTableBulk() {
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      setError("");
      try {
        const res = await axios.get(BULK_API);
        if (!cancelled) {
          setRows(Array.isArray(res.data) ? res.data : []);
        }
      } catch (err) {
        if (!cancelled) {
          setError("Failed to load bulk process data");
          console.error(err);
        }
      } finally {
          ...
          <Table size="small">
            ...
            <TableBody>
              {rows.map((row) => {
                ...
                const statusLabel =
                  STATUS_LABELS[row.processStatus] ?? row.processStatus;

                return (
                  <TableRow key={row.processId ?? row.id}>
              ...
              {rows.length === 0 && (
                <TableRow>
                  <TableCell colSpan={7} align="center">
                    No bulk process data available.
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
...
export default ProcessTableBulk;
